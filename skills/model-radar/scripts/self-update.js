...
const DEFAULT_BRANCH = process.env.MODEL_RADAR_UPDATE_REF || "main";
...
const DATA_CACHE_FILES = ["raw.json", "models.json", "rankings.json"];
...
function warn(message) {
  console.error(`[warn] model-radar self-update: ${message}`);
}
...
async function fetchJson(url) {
  const response = await fetch(url, {
    headers: {
      ...
      "user-agent": "model-radar-self-update"
    },
...
function ttlHours() {
  const raw = process.env.MODEL_RADAR_AUTO_UPDATE_TTL_HOURS || "24";
...
function tempRoot() {
  ...
  return join(root, `model-radar-update-${process.pid}-${Date.now()}`);
}
...
async function auto() {
  const autoEnv = (process.env.MODEL_RADAR_AUTO_UPDATE || "").toLowerCase();
  if (AUTO_UPDATE_OFF.has(autoEnv)) return { command: "auto", status: "skipped", reason: "disabled" };

  const { repoUrl, directory } = packageInfo();
  if (isSourceCheckout(repoUrl, directory) && process.env.MODEL_RADAR_AUTO_UPDATE_SOURCE !== "1") {
...
function status() {
  ...
  return {
    ...
    auto_update: !AUTO_UPDATE_OFF.has((process.env.MODEL_RADAR_AUTO_UPDATE || "").toLowerCase()),
    ttl_hours: ttlHours()
  };
}
    ...
    switch (command) {
      ...
      case "auto": {
        ...
        if (result.status === "updated") {
          console.error(`MODEL_RADAR_UPDATED revision=${result.revision?.slice(0, 12)} cache_cleared=${(result.cache_cleared || []).join(",")}`);
