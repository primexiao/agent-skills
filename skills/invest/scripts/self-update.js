...
const DEFAULT_BRANCH = process.env.INVEST_UPDATE_REF || "main";
...
function warn(message) {
  console.error(`[warn] invest self-update: ${message}`);
}
...
function ttlHours() {
  const raw = process.env.INVEST_AUTO_UPDATE_TTL_HOURS || "24";
...
function tempRoot() {
  ...
  return join(root, `invest-update-${process.pid}-${Date.now()}`);
}
...
async function auto() {
  const autoEnv = (process.env.INVEST_AUTO_UPDATE || "").toLowerCase();
  if (AUTO_UPDATE_OFF.has(autoEnv)) return { command: "auto", status: "skipped", reason: "disabled" };

  const { repoUrl, directory } = packageInfo();
  if (isSourceCheckout(repoUrl, directory) && process.env.INVEST_AUTO_UPDATE_SOURCE !== "1") {
...
function status() {
  ...
  return {
    ...
    auto_update: !AUTO_UPDATE_OFF.has((process.env.INVEST_AUTO_UPDATE || "").toLowerCase()),
    ttl_hours: ttlHours()
  };
}
    ...
    switch (command) {
      ...
      case "auto": {
        ...
        if (result.status === "updated") {
          console.error(`INVEST_UPDATED revision=${result.revision?.slice(0, 12)} cache_cleared=${(result.cache_cleared || []).join(",")}`);
