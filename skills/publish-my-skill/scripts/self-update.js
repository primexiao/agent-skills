...
const DEFAULT_BRANCH = process.env.PUBLISH_MY_SKILL_UPDATE_REF || "main";
...
function warn(message) {
  console.error(`[warn] publish-my-skill self-update: ${message}`);
}
...
async function fetchJson(url) {
  const response = await fetch(url, {
    headers: {
      ...
      "user-agent": "publish-my-skill-self-update"
    },
...
function ttlHours() {
  const raw = process.env.PUBLISH_MY_SKILL_AUTO_UPDATE_TTL_HOURS || "24";
...
function tempRoot() {
  ...
  return join(root, `publish-my-skill-update-${process.pid}-${Date.now()}`);
}
...
async function auto() {
  const autoEnv = (process.env.PUBLISH_MY_SKILL_AUTO_UPDATE || "").toLowerCase();
  if (AUTO_UPDATE_OFF.has(autoEnv)) return { command: "auto", status: "skipped", reason: "disabled" };

  const { repoUrl, directory } = packageInfo();
  if (isSourceCheckout(repoUrl, directory) && process.env.PUBLISH_MY_SKILL_AUTO_UPDATE_SOURCE !== "1") {
...
function status() {
  ...
  return {
    ...
    auto_update: !AUTO_UPDATE_OFF.has((process.env.PUBLISH_MY_SKILL_AUTO_UPDATE || "").toLowerCase()),
    ttl_hours: ttlHours()
  };
}
    ...
    switch (command) {
      ...
      case "auto": {
        ...
        if (result.status === "updated") {
          console.error(`PUBLISH_MY_SKILL_UPDATED revision=${result.revision?.slice(0, 12)}`);
